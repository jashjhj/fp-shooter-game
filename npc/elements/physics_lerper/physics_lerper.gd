...
func apply_forces(delta:float) -> void:
	...
	var delta_pos = AUTO_TRIGGER_NODE.global_position - RIGIDBODY.global_position
	...
	var velocity_along_delta = RIGIDBODY.linear_velocity.dot(delta_pos.normalized()) * delta_pos.normalized()
	...
	#var lost_impulse = last_force*delta - RIGIDBODY.mass*(RIGIDBODY.linear_velocity - last_velocity)
	...
	last_force = force_to_apply;
	
	
	
	
	
	RIGIDBODY.apply_central_force(force_to_apply)

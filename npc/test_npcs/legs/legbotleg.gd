...
func attempt_apply_force(force:Vector3):
	...
	if((loc_force * Vector3(0, 1, 1)).is_zero_approx()): # If its zero, cancel and move on
		...
		KNEE.set("angular_motor_x/target_velocity", ik_delta_angle_knee * 1); # Previosuly these were sign(ik_delta_angle) with no magnitude
		
		HIP.set("angular_motor_x/enabled", true);
		HIP.set("angular_motor_x/target_velocity", ik_delta_angle_hip * 1);

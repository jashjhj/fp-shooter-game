...
func _ready() -> void:
	IKCALC.UPPER_LENGTH = 1.0;
	IKCALC.LOWER_LENGTH = 1.0;
...
func is_on_floor() -> bool:
	...
	for body in contacts:
		if(body is StaticBody3D): # If foot is colliding with a staticbody, YES is on floor
			return true;
...
func attempt_apply_force(force:Vector3):
	var loc_force:Vector3 = force * global_basis.inverse();
	
	if((loc_force * Vector3(0, 1, 1)).is_zero_approx()): # If its zero, cancel and move on
		KNEE.set("angular_motor_x/enabled", false);
		HIP.set("angular_motor_x/enabled", true);
	else:
		#Find directions of travel
		
		var ik_considered_delta = (to_local(FOOT.global_position) - loc_force.normalized()) * 0.1 * Vector3(0, 1, 1); # Omit X as this can only be fixed via hip yaw. Quantized to small amounts for simple delta-angles as per calculations
		var angles = IKCALC.get_angles(ik_considered_delta, Vector3.DOWN)
		
		var angle_hip = UPPER.rotation.x + PI/2
		var angle_knee = LOWER.rotation.x - angle_hip;
		...
		KNEE.set("angular_motor_x/target_velocity", sign(ik_delta_angle_knee) * 1);
		
		HIP.set("angular_motor_x/enabled", true);
		HIP.set("angular_motor_x/target_velocity", sign(ik_delta_angle_hip) * 1);
		...
		var moment_hip = (loc_force.z + loc_force.y) / (sin(angle_hip)*cos(angle_knee)/sin(angle_knee) + cos(angle_hip))
		var moment_knee = (loc_force.z - moment_hip*cos(angle_hip)) / cos(angle_knee)
		
		moment_hip *= IKCALC.UPPER_LENGTH;
		moment_knee *= IKCALC.LOWER_LENGTH;
		
		KNEE.set("angular_motor_x/force_limit_x", abs(moment_knee))
		HIP.set("angular_motor_x/force_limit_x", abs(moment_hip))
		
		
		HIP.set("angular_motor_y/enabled", true);
		HIP.set("angular_motor_y/target_velocity", -sign(loc_force.y) * 1.0)

...
##Should not be child of Upper-leg, but a sibling.
@export var LOWER_LEG:Node3D;
@export var UPPER_LENGTH:float = 0.2;
@export var LOWER_LENGTH:float = 0.4;
...
# On ground makes it interpolate linearly, otherwise adds a curve to it.
...
#Default pos @ start
@onready var prev_pos:Vector3 = global_position + Vector3(0, UPPER_LENGTH - LOWER_LENGTH, 0);
...
func set_leg_ik(pos:Vector3) -> float:
	...
	LOWER_LEG.position = -UPPER_LEG.basis.z*UPPER_LENGTH

...
func _physics_process(delta:float) -> void:
	...
	if(is_focused): #Manual controls
		...
		#
		##Initial checks:
		#if(DISTANCE <= functional_min and velocity >= 0): # Previously...
			#DISTANCE = functional_min
			#velocity = 0;
			#if(forces > 0): # if forces will push it away ,
				#velocity += forces*delta;
				#
		#elif(DISTANCE >= functional_max and velocity <= 0):
		...
		#DISTANCE += velocity
		#
		#if(DISTANCE <= functional_min and velocity < 0): # if JUST hit the wall
			#DISTANCE = functional_min;
			#
			#hit_min_angle(velocity)
			#
			#velocity *= -ELASTICITY_TOP;
		#
		#if(DISTANCE >= functional_max and velocity > 0): # if JUST hit the wall
			#DISTANCE = functional_max;
			#
			#hit_max_angle(velocity)
			#
			#velocity *= -ELASTICITY_TOP;

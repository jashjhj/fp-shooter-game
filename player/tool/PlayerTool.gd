...
func _process(delta: float) -> void:
	...
	var inspect_rot := Quaternion(MODEL_POSITION_INSPECT.transform.basis);
	
	var lerp_rate := 0.1;
	
	if(inspect):
		MODEL.basis = current_rot.slerp(inspect_rot, lerp_rate);
		MODEL.position = MODEL.position.lerp(MODEL_POSITION_INSPECT.position, lerp_rate)
	else:
...
func rotate_inspect_node(rot: Vector2):
	MODEL_POSITION_INSPECT.rotate_y(rot.x)
	MODEL_POSITION_INSPECT.rotate_x(rot.y)
	
	MODEL.basis = Quaternion(MODEL.transform.basis).slerp(Quaternion(MODEL_POSITION_INSPECT.transform.basis), 0.1); # increases responsiveness
